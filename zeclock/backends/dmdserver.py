"""
DMDServer TCP backend implementing the DMDBackend ABC.

Protocol: DMDStream header (25 bytes) + RGB565 data (big-endian).
Communicates with dmdserver (libdmdutil) or virtual-dmd.py.
"""
...
class DMDServerBackend(DMDBackend):
    """TCP client backend for dmdserver (DMDStream protocol).

    Sends RGB565 frames over TCP to a dmdserver instance or virtual-dmd.py.
    Implements frame identity caching to skip re-encoding identical frames,
    and uses pre-computed RGB565 LUTs for fast pixel conversion.
    """
    ...
    def connected(self) -> bool:
        """Whether the backend is currently connected to dmdserver."""
        return self._connected

    def connect(self) -> bool:
        """Establish TCP connection to dmdserver."""
        try:
    ...
    def send_frame(
        ...
        """Send an RGB565 frame to dmdserver.

        Uses frame identity caching: if the same Image object is sent again,
        skips colorization and RGB565 conversion entirely.

        If the TCP connection is lost, attempts reconnection. If reconnection
        fails, returns False and reattempts on the next call.
        """
        ...
        try:
            ...
            logger.warning("Error sending frame to dmdserver: %s", e)
            # Connection lost — mark as disconnected so next call attempts reconnect
            self._connected = False
            return False
